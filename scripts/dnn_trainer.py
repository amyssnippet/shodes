...
class DNNTrainer:
    ...
    def train(self,epochs=1000):
        ...
        for model_type in self.models:
            ...
            pinns=[tf.keras.models.load_model(f'models/{model_type}_pinss/pinn_{i+1}') for i in range(self.n)]
            for epoch in range(epochs):
                with tf.GradientTape() as tape:
                    pinn_outputs=[pinn.predict(x_data[:,i:i+1],verbose=0) for i,oinn in enumerate(pinns)]
                    pinn_outputs=tf.concat(pinn_outputs,axis=1)
                    dnn_input=tf.concat([pinn_outputs,tf.repeat([[self.k,self.kc,self.lambda_]],repeats=len(x_data),axis=0)],axis=1)
                    V_pred=self.dnns[model_type](dnn_input)
                    V_true_tensor=tf.convert_to_tensor(V_true,dtype=float32)
                    loss=tf.reduce_mean(tf.square(V_pred-V_true_tensor))
            ...
            self.dnns[model_type].save(f'models/{model_type}_dnn')
