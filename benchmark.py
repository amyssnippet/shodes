...
def simulate_kuramoto_2osc(T=100.0, dt=0.1, K=0.1, omega=None, theta0=None):
    steps = int(T / dt)
...
def simulate_kuramoto_strogatz_2osc(T=100.0, dt=0.1, K=0.1, omega=None, theta0=None, p=0.5):
    n = 2
...
def run_shodes_pinn_dnn(batch_size=1001, n_oscillators=2, model_type='genpot3', params=[1.0, 0.1, 0.1]):
    ...
    time_ml, theta_ml = simulate_kuramoto_2osc(T=100.0, dt=0.1, K=params[2])
    x_data = np.sin(theta_ml[:batch_size]).astype(np.float32)
...
def plot_benchmark_with_dnn(time_k, R_kuramoto, time_s, R_strogatz, time_ml, pinn_outputs, dnn_energy):
    ...
    axs[1].plot(time_ml, pinn_outputs[:, 0], '--', label='SHODes PINN Oscillator 1')
    axs[1].plot(time_ml, pinn_outputs[:, 1], '--', label='SHODes PINN Oscillator 2')
    axs[1].plot(time_ml, dnn_energy.squeeze(), ':', label='SHODes DNN Predicted Energy')
...
def main():
    time_k, theta_k = simulate_kuramoto_2osc(T=100.0, dt=0.1, K=0.1)
    R_kuramoto = np.array([kuramoto_order_parameter(theta_k[t]) for t in range(len(time_k))])
    time_s, theta_s = simulate_kuramoto_strogatz_2osc(T=100.0, dt=0.1, K=0.1, p=0.5)
    R_strogatz = np.array([kuramoto_order_parameter(theta_s[t]) for t in range(len(time_s))])
    batch_size = len(time_k)
    time_ml, pinn_outputs, dnn_energy = run_shodes_pinn_dnn(batch_size=batch_size)
